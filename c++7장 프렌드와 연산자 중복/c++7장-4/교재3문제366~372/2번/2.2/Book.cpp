...
bool operator==(Book& op ,int a ){                               // operator==함수 정의
    if (a == op.price) return true;                              // 조건문이 참이면 true를 반환
    else return false;                                           // 거짓이면 false를 반환
}

bool operator==(Book& op, string a){                             // operator==함수 정의
    if( a== op.title) return true;                               // 조건문이 참이면 true를 반환                
    else return false;                                           // 거짓이면 false를 반환
}

bool operator==(Book& op,Book& op2 ){                                                     // operator==함수 정의
    if((op.title==op2.title)&&(op.price==op2.price)&&(op.pages==op2.pages)) return true;  // 조건문이 참이면 ture를 반환
